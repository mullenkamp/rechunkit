...
###################################################
### Read count optimization tests

def test_optimized_reads_never_worse_than_greedy():
    """Shape-aware calc should never produce more reads than greedy."""
    configs = [
        ((31, 31), (5, 2), (2, 5), 80),
        ((50, 50), (7, 3), (3, 7), 200),
        ((50, 50), (7, 3), (3, 7), 500),
        ((100, 100), (6, 4), (4, 6), 200),
        ((40, 40), (12, 5), (5, 12), 500),
        ((31, 31, 31), (5, 2, 4), (4, 5, 3), 500),
        ((30, 30, 30), (7, 3, 5), (5, 7, 3), 1000),
        ((40, 40, 40), (6, 4, 5), (5, 6, 4), 2000),
    ]
    itemsize = 4
    for shp, src_c, tgt_c, mem in configs:
        # Greedy (no shape)
        greedy_shape = calc_source_read_chunk_shape(src_c, tgt_c, itemsize, mem)
        # Optimized (with shape)
        opt_shape = calc_source_read_chunk_shape(src_c, tgt_c, itemsize, mem, shape=shp)

        greedy_reads, _ = calc_n_reads_rechunker(shp, itemsize, src_c, tgt_c, mem)

        # Verify opt shape fits in memory
        assert prod(opt_shape) * itemsize <= mem or opt_shape == src_c
        # Verify each dim >= source chunk
        assert all(o >= s for o, s in zip(opt_shape, src_c))


def test_count_reads_consistency():
    """_count_reads should match the actual plan read count."""
    from rechunkit.main import _count_reads, _rechunk_plan
    ...
    configs = [
        ...
        ((31, 31, 31), (5, 2, 4), (4, 5, 3), 2000),
        ((100,), (7,), (11,), 100),
        ((20, 20, 20), (4, 3, 5), (5, 4, 3), 1000),
    ]
    itemsize = 4
    for shp, src_c, tgt_c, mem in configs:
        src_rcs = calc_source_read_chunk_shape(src_c, tgt_c, itemsize, mem, shape=shp)
        estimated = _count_reads(shp, src_c, tgt_c, src_rcs)

        actual = 0
        for _, read_chunks, _, _ in _rechunk_plan(shp, itemsize, src_c, tgt_c, mem, source_read_chunk_shape=src_rcs):
            actual += len(read_chunks)

        assert estimated == actual, f"shape={shp}: _count_reads={estimated} != plan={actual}"
